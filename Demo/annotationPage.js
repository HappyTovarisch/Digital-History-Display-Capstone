...
async function showAnnotations() {
	for (let thing in person) {
		if ((thing.match(/myURL/) != null) || (thing.match(/param/) != null) || (thing.match(/id/) != null)) {
			console.log(thing);
			console.log(person[thing]);
		}
		else if (person[thing].value == undefined) {}
		else if (thing.match(/birthDate/)) {
			document.getElementById(`${thing}`).value=person[thing].value;
		}
		else if (thing.match(/deathDate/) != null) {
			document.getElementById(`${thing}`).value=person[thing].value;
		}
		else if (thing.match(/givenName/) != null) {
			document.getElementById(`newannotationvalue`).value=person[thing].value;
		}
		else if (thing.match(/familyName/) != null) {
			document.getElementById(`${thing}`).value=person[thing].value;
		}
...
async function updateAnnotation() {
  ...
  newAnnotation = await populateAnnotation(newAnnotation); //only one that touches those boxes
